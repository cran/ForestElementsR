...
listed in the documentation, and you can look them up with ```?yield_tables```. 
...
Let us have a look at a classic yield table, Wiedemann's 1943 table for Scots
...
actually represented in the table with values. The Wiedemann pine table uses
...
variable names given above, beginning with those that are common in most yield 
tables:
...
* **tvp_m3_ha**: Total volume production (m³/ha)
* **mai_m3_ha_yr**: Mean annual volume increment (m³/ha/yr), i.e. the total 
