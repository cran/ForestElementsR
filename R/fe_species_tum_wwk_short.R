...
# Which reflects the short tree species coding version used at the Chair for
# Forest Growth and Yield Science at th Technical University of Munich
...
#'
#' @return An object of class \code{fe_species_tum_wwk_short}
#'
#' @export
#'
#' @examples
...
#' )
#'
new_fe_species_tum_wwk_short <- function(x = character()) {
  new_fe_species(x, "fe_species_tum_wwk_short")
}
...
#'   \code{fe_species_tum_wwk_short} class
#'
#' @export
#'
#' @examples
...
#'
is_fe_species_tum_wwk_short <- function(x) {
  inherits(x, "fe_species_tum_wwk_short")
}
...
#' \code{fe_species_tum_wwk_short} is printed, be it alone, be it as part of
#' another object (e.g. a tibble)
#'
#' @param x An object of type \code{fe_species_tum_wwk_short}
#'
...
#' spec_ids
#'
format.fe_species_tum_wwk_short <- function(x,
...
#'
#' Produces a summary for a fe_species_tum_wwk_short object in the same style as
...
#'
#' @param object Object of class \code{\link{fe_species_tum_wwk_short}}
#'
...
#'
summary.fe_species_tum_wwk_short <- function(object,
                                             spec_lang = options("fe_spec_lang")$fe_spec_lang,
                                             maxsum = 100L,
                                             ...) {
...
#'
#' Provide an abbreviated name for the class \code{fe_species_tum_wwk_short} to
...
#' str(spec_ids)
#'
vec_ptype_abbr.fe_species_tum_wwk_short <- function(x, ...) {
  "tm_wwk_shrt"
}
...
#' )
#'
validate_fe_species_tum_wwk_short <- function(x = character()) {
  stopifnot(is_fe_species_tum_wwk_short(x)) # Check for class attribute
...
#' *fe_species_tum_wwk_short* convention
#'
#' The *tum_wwk_short* species coding is one of two codings in use
...
#'   \code{fe_species_tum_wwk_short} object, this object will be returned. If
...
#'
fe_species_tum_wwk_short <- function(x = character()) {
  x <- trimws(as.character(x), which = "both")
  x <- new_fe_species_tum_wwk_short(x)
  validate_fe_species_tum_wwk_short(x)
}


# A fe_species_tum_wwk_short should be ordered by its integer values, even if
# ordering by the output of format would suggest another order

#' @export
vec_proxy_order.fe_species_tum_wwk_short <- function(x, ...) {
  as.integer(vctrs::vec_data(x))
}


# Casting into fe_species_tum_wwk_short

#' @export
vec_cast.fe_species_tum_wwk_short.integer <-
  function(x, to, ...) {
    x <- as.character(x)
    fe_species_tum_wwk_short(x)
  }

#' @export
vec_cast.fe_species_tum_wwk_short.double <-
  function(x, to, ...) {
    x <- as.character(x)
    fe_species_tum_wwk_short(x)
  }
...
#' @export
vec_cast.fe_species_tum_wwk_short.character <-
  function(x, to, ...) {
    fe_species_tum_wwk_short(x)
  }
...
#' @export
vec_cast.fe_species_tum_wwk_short.fe_species_bavrn_state <-
  function(x, to, ...) {
    x_trans <- spec_id_cast_do_it(x, "bavrn_state", "tum_wwk_short")
    fe_species_tum_wwk_short(x_trans)
  }

#' @export
vec_cast.fe_species_tum_wwk_short.fe_species_bavrn_state_short <-
  function(x, to, ...) {
    x_trans <- spec_id_cast_do_it(x, "bavrn_state_short", "tum_wwk_short")
    fe_species_tum_wwk_short(x_trans)
  }

#' @export
vec_cast.fe_species_tum_wwk_short.fe_species_tum_wwk_long <-
  function(x, to, ...) {
    x_trans <- spec_id_cast_do_it(x, "tum_wwk_long", "tum_wwk_short")
    fe_species_tum_wwk_short(x_trans)
  }

#' @export
vec_cast.fe_species_tum_wwk_short.fe_species_master <-
  function(x, to, ...) {
    x_trans <- spec_id_cast_do_it(x, "master", "tum_wwk_short")
    fe_species_tum_wwk_short(x_trans)
  }



#' Cast Appropriate Objects Into a **fe_stand_tum_wwk_short** Species Class
#' Object
#'
...
#'   \code{fe_species_tum_wwk_short} object is returned
#'
#' @export
#'
#' @examples
...
#'
as_fe_species_tum_wwk_short <- function(x) {
  vec_cast(x, to = fe_species_tum_wwk_short())
}
