...
test_that("height calculation h_standard_gnfi3 is correct", {
  ...
  # Check for bavrn_state species coding: three trees per (tree) species. The
  # function casts bavrn_state -> bavrn_state_short, so each species must give
  # exactly its short group's value. Tested as that invariant (order- and
  # edit-robust); absolute values are pinned in the bavrn_state_short block above.
  ...
  expect_equal(
    suppressWarnings(h_standard_gnfi3(spec, d_cm, dq, hq)),
    suppressWarnings(h_standard_gnfi3(
      as_fe_species_bavrn_state_short(spec), d_cm, dq, hq
    ))
  )


  # Check for tum_wwk_long species coding: three trees per (tree) species. The
  # function casts tum_wwk_long -> tum_wwk_short (a valid coarsening), so each
  # species must give exactly its short group's value. Tested as that invariant
  # (order- and edit-robust); absolute values are pinned in the tum_wwk_short
  # block above.
  ...
  expect_equal(
    suppressWarnings(h_standard_gnfi3(spec, d_cm, dq, hq)),
    suppressWarnings(h_standard_gnfi3(
      as_fe_species_tum_wwk_short(spec), d_cm, dq, hq
    ))
  )
})
